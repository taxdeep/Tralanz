#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Citus auto-bump (four-segment scheme, 2026-06-18).
#
...
#   Segment 3  XX   decimal, 2 chars  (00-99)
#   Segment 4  YY   base-36, 2 chars  (00-ZZ, 36^2 = 1 296 values)
#
# This is the old X.XX.XXX.YYYY.XX.YY scheme with the two wide middle
# segments (.XXX.YYYY) dropped — the carry chain and per-push semantics
# are otherwise unchanged. Every push bumps segment 4 (the rightmost YY)
# by +1 and carries left on overflow:
#
#   YY      ZZ -> 00,   segment 3 += 1
#   XX (3)  99 -> 00,   segment 2 += 1
...
# capped at 65 534, and the base-36 YY slot (plus the uncapped segment 1)
# can't be embedded there. Keeping AssemblyVersion stable also avoids
...
readonly SEG3_DEC_MAX=99   # 10^2 - 1
readonly SEG4_B36_MAX=1295 # 36^2 - 1
...
read_current_version() {
  ...
  if ! [[ "${version}" =~ ^[0-9]+\.[0-9]{2}\.[0-9]{2}\.[0-9A-Za-z]{2}$ ]]; then
    echo "Unsupported version format in ${VERSION_FILE}: '${version}' (expected X.XX.XX.YY with widths 1+/2/2/2)." >&2
    exit 1
  fi
...
bump_version() {
  ...
  local seg1_dec seg2_dec seg3_dec seg4_dec
  seg1_dec=$(( 10#${seg1} ))
  seg2_dec=$(( 10#${seg2} ))
  seg3_dec=$(( 10#${seg3} ))
  ...
  local next_seg2 next_seg3 next_seg4
  printf -v next_seg2 '%02d' "${seg2_dec}"
  printf -v next_seg3 '%02d' "${seg3_dec}"
  next_seg4="$(b36_encode "${seg4_dec}" 2)"

  printf '%d.%s.%s.%s' "${seg1_dec}" "${next_seg2}" "${next_seg3}" "${next_seg4}"
}
