...
public static class BillReceiptPostingGate
{
    ...
    public static string GetPostingGateSummary(InventoryBillReceiptHandoffSummary? summary) =>
        summary?.MatchStatus switch
        {
            ...
            "partially_receipted" => $"AP posting stays on hold until the remaining {summary.RemainingQuantity:N2} inbound quantity is covered by receipt truth.",
    ...
    public static string GetBlockedPostMessage(InventoryBillReceiptHandoffSummary summary) =>
        summary.MatchStatus switch
        {
            ...
            "partially_receipted" => $"Bill posting is on hold because receipt truth still has {summary.RemainingQuantity:N2} quantity outstanding against this bill hand-off.",
