...
public static class BusinessApprovalAuthority
{
    ...
    public static bool CanViewAccounts(BusinessSessionContext? session)
    {
        if (session is null)
        {
            return false;
        ...
        return session.Roles.Any(role =>
        {
            ...
            return AccountingPostRoles.Contains(normalized, StringComparer.Ordinal) ||
                   SalesPostRoles.Contains(normalized, StringComparer.Ordinal) ||
                   PurchasePostRoles.Contains(normalized, StringComparer.Ordinal) ||
                   ArPaymentPostRoles.Contains(normalized, StringComparer.Ordinal) ||
                   ApPaymentRolesContains(normalized) ||
                   InventoryOperationRoles.Contains(normalized, StringComparer.Ordinal) ||
                   TaskOperationRoles.Contains(normalized, StringComparer.Ordinal) ||
                   BankingOperationRoles.Contains(normalized, StringComparer.Ordinal);
        });
    }

    private static bool ApPaymentRolesContains(string normalizedRole) =>
        ApPaymentPostRoles.Contains(normalizedRole, StringComparer.Ordinal);
