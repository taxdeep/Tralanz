...
/// <summary>
/// P-20 — accounting-truth coverage for the invoice POST path. The
/// existing receivable smoke tests only flip status + insert a fake
/// JE; here we drive a real <see cref="InvoiceDocument"/> through the
/// production <see cref="DefaultPostingEngine"/> (wired exactly as
/// Program.cs wires it) inside a <see cref="PostgresUnitOfWork"/> — the
/// same Step 1 mechanism PostInvoiceCommandHandler uses — and then read
/// the persisted journal_entry_lines straight back to assert the entry
/// is balanced on BOTH axes (Σdebit = Σcredit AND Σtx_debit = Σtx_credit)
/// and that the expected control / revenue / tax-payable accounts were
/// hit. SalesTaxV2 is enabled on the draft save so the engine computes
/// GST 5% and the fragment builder emits the per-snapshot tax-payable
/// leg from the tax code's component payable account.
/// </summary>
public sealed class InvoicePostingBalanceSmokeTests
{
    private static readonly CompanyId CompanyId = CompanyId.FromOrdinal(1);
    private static readonly Guid CustomerId = Guid.Parse("91000000-0000-0000-0000-000000000002");

    [SkippableFact]
    public async Task PostInvoice_WithSalesTaxV2_WritesBalancedJournalEntryHittingArRevenueAndTaxPayable()
    {
        ...
        // The repository under SalesTaxV2: the engine (not the operator) is
        // the tax authority and writes the per-line snapshot that the
        // posting fragment builder later reads.
        ...
        Guid taxPayableAccountId = default;
        ...
        try
        {
            ...
            revenueAccountId = await CreateAccountAsync(
                connectionFactory, CompanyId, "revenue", "revenue", "REV", "Smoke Revenue (P-20)", systemRole: null, CancellationToken.None);
            taxPayableAccountId = await CreateAccountAsync(
                connectionFactory, CompanyId, "liability", "sales_tax_payable", "TAXP", "Smoke Sales Tax Payable (P-20)", systemRole: null, CancellationToken.None);
            (legacyTaxCodeId, salesTaxCodeId) = await CreateGstFivePercentTaxCodeAsync(
                connectionFactory, CompanyId, taxPayableAccountId, recoverableAccountId: null, CancellationToken.None);

            // $100 taxable line; engine computes GST 5% = $5.00. Operator-
            // entered tax is 0 — the engine is the authority under v2.
            ...
            // Re-load through the posting repository to learn the routed AR
            // control account (the demo COA may carry several
            // accounts_receivable-role accounts; we assert against the one
            // the engine will actually post to rather than guessing).
            var document = await invoiceRepository.GetForPostingAsync(CompanyId, invoiceId, CancellationToken.None);
            Assert.NotNull(document);
            Assert.Equal(105.00m, document!.TotalAmount);
            Assert.Equal(5.00m, document.TaxAmount);
            var receivableAccountId = document.ReceivableAccountId;

            // Drive the REAL posting engine inside a unit of work, exactly
            // like PostInvoiceCommandHandler Step 1 (the JE writer demands an
            // ambient transaction, so the UoW wrapper is mandatory).
            ...
            // Same-currency post → tx and base columns coincide.
            Assert.Equal(105.00m, lines.Sum(l => l.Debit));
            Assert.Equal(105.00m, lines.Sum(l => l.Credit));

            // --- The expected accounts/roles were hit. ---
            // AR control debited the gross 105 (subtotal + tax).
            var arLine = Assert.Single(lines, l => l.AccountId == receivableAccountId);
            Assert.Equal("accounts_receivable", arLine.ControlRole);
            Assert.Equal(105.00m, arLine.Debit);
            Assert.Equal(0m, arLine.Credit);

            // Revenue credited the net 100.
            var revenueLine = Assert.Single(lines, l => l.AccountId == revenueAccountId);
            Assert.Equal(0m, revenueLine.Debit);
            Assert.Equal(100.00m, revenueLine.Credit);

            // Tax payable credited the engine-computed 5, stamped with the
            // sales_tax_payable component type.
            var taxLine = Assert.Single(lines, l => l.AccountId == taxPayableAccountId);
            Assert.Equal("sales_tax_payable", taxLine.TaxComponentType);
        ...
        finally
        {
            ...
            await CleanupAccountAsync(connectionFactory, taxPayableAccountId, CancellationToken.None);
    ...
    // DefaultPostingValidator + PostgresPostingPeriodPolicyValidator +
    // NullTaxEngine + LocalFirstFxResolutionService(PostgresFxSnapshotRepository)
    // + AccountingPostingFragmentBuilder + DefaultJournalAggregator +
    // PostgresJournalEntryWriter. Tax does NOT come from the NullTaxEngine —
    // it comes from the per-line snapshots the fragment builder reads.
        ...
        CancellationToken cancellationToken)
    {
        ...
        insertCommand.Parameters.AddWithValue("email", $"p20-{newUserId.Value}@tralanz.local");
        insertCommand.Parameters.AddWithValue("username", $"p20-{newUserId.Value}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            ...
            insert into accounts (
              ...
              is_active, is_system, is_system_default, system_role,
              allow_manual_posting, created_at, updated_at)
            values (
              @id, @company_id, @entity_number, @code, @name,
              @root_type, @detail_type,
              true, false, false, @system_role,
              true, now(), now());
            """;
        ...
        command.Parameters.AddWithValue("system_role", (object?)systemRole ?? DBNull.Value);
        await command.ExecuteNonQueryAsync(cancellationToken);
        return accountId;
    }

    // Single-component GST 5% tax code (legacy + v2). The component carries
    // the payable (and optionally recoverable) GL account so the posting
    // fragment builder can route the per-snapshot tax leg. CA GST
        ...
        await using (var componentCommand = connection.CreateCommand())
        {
            componentCommand.CommandText =
                ...
                insert into sales_tax_code_components (
                  ...
                  payable_account_id, recoverable_account_id,
            ...
            componentCommand.Parameters.AddWithValue("payable_account_id", payableAccountId);
            componentCommand.Parameters.AddWithValue("recoverable_account_id", (object?)recoverableAccountId ?? DBNull.Value);
        ...
        CancellationToken cancellationToken)
    {
        ...
        throw new InvalidOperationException("Could not reserve a unique entity number for invoice posting smoke test.");
    }
