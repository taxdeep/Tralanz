...
public sealed class ShellCompanyNumberingClient(PostgreSqlConnectionFactory connections)
{
    private static readonly IReadOnlyList<ShellCompanyNumberingDefinition> KnownDefinitions =
    [
    ...
    public async Task<ShellCompanyNumberingSummary> GetSummaryAsync(
        Guid companyId,
        CancellationToken cancellationToken = default)
    {
        ...
        var merged = new List<ShellCompanyNumberingRuleSummary>();
        ...
        foreach (var definition in KnownDefinitions)
        {
            ...
            merged.Add(new ShellCompanyNumberingRuleSummary(
                definition.Module,
                definition.Label,
                definition.ScopeKey,
                definition.Family,
        ...
        return new ShellCompanyNumberingSummary
        {
            CompanyId = companyId,
            Rules = merged
    ...
    public async Task<ShellCompanyNumberingRuleSummary> SaveAsync(
        Guid companyId,
        ShellCompanyNumberingRuleUpdateRequest request,
        CancellationToken cancellationToken = default)
    {
        Validate(request);
        ...
        return new ShellCompanyNumberingRuleSummary(
            definition.Module,
            definition.Label,
            definition.ScopeKey,
            definition.Family,
    ...
    private static async Task<IReadOnlyList<ShellCompanyNumberingRow>> LoadRowsAsync(
        NpgsqlConnection connection,
        Guid companyId,
        CancellationToken cancellationToken)
    {
        ...
        var rows = new List<ShellCompanyNumberingRow>();
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            rows.Add(new ShellCompanyNumberingRow(
    ...
    private static void Validate(ShellCompanyNumberingRuleUpdateRequest request)
    {
    ...
    private sealed record ShellCompanyNumberingDefinition(
        string Module,
        string Label,
        string ScopeKey,
    ...
    private sealed record ShellCompanyNumberingRow(
        string ScopeKey,
        string Prefix,
        long NextNumber,
        short Padding,
...
public sealed record ShellCompanyNumberingSummary
{
    public Guid CompanyId { get; init; }
    public IReadOnlyList<ShellCompanyNumberingRuleSummary> Rules { get; init; } = Array.Empty<ShellCompanyNumberingRuleSummary>();
}

public sealed record ShellCompanyNumberingRuleSummary(
    string Module,
    string Label,
    string ScopeKey,
    string Family,
...
public sealed record ShellCompanyNumberingRuleUpdateRequest(
    string ScopeKey,
    string Prefix,
    short Padding,
    long NextNumber,
