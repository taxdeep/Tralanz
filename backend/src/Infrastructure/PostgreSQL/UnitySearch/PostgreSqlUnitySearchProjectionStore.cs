    ...
    private static async Task SeedInvoiceDocumentsAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        await ExecuteCompanyProjectionStepAsync(
            ...
            select
              ...
              'transactions',
              i.display_number,
              concat_ws(' | ', i.status, coalesce(i.transaction_currency_code, ''), coalesce(c.display_name, '')),
              concat_ws(' ', i.display_number, i.entity_number, i.status, coalesce(c.display_name, ''), coalesce(i.customer_reference, '')),
              to_tsvector('simple', concat_ws(' ', i.display_number, i.entity_number, i.status, coalesce(c.display_name, ''), coalesce(i.customer_reference, ''))),
              lower(coalesce(i.entity_number, i.display_number)),
    ...
    private static async Task SeedBillDocumentsAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        await ExecuteCompanyProjectionStepAsync(
            ...
            select
              ...
              'transactions',
              b.display_number,
              concat_ws(' | ', b.status, coalesce(b.transaction_currency_code, ''), coalesce(v.display_name, '')),
              concat_ws(' ', b.display_number, b.entity_number, b.status, coalesce(v.display_name, ''), coalesce(b.vendor_reference, '')),
              to_tsvector('simple', concat_ws(' ', b.display_number, b.entity_number, b.status, coalesce(v.display_name, ''), coalesce(b.vendor_reference, ''))),
              lower(coalesce(b.entity_number, b.display_number)),
    ...
    private static async Task SeedCreditNoteDocumentsAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        await ExecuteCompanyProjectionStepAsync(
            ...
            select
              ...
              'transactions',
              note.display_number,
              concat_ws(' | ', note.status, coalesce(note.transaction_currency_code, ''), coalesce(c.display_name, '')),
              concat_ws(' ', note.display_number, note.entity_number, note.status, coalesce(c.display_name, '')),
              to_tsvector('simple', concat_ws(' ', note.display_number, note.entity_number, note.status, coalesce(c.display_name, ''))),
              lower(coalesce(note.entity_number, note.display_number)),
    ...
    private static async Task SeedVendorCreditDocumentsAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        await ExecuteCompanyProjectionStepAsync(
            ...
            select
              ...
              'transactions',
              vc.display_number,
              concat_ws(' | ', vc.status, coalesce(vc.transaction_currency_code, ''), coalesce(v.display_name, '')),
              concat_ws(' ', vc.display_number, vc.entity_number, vc.status, coalesce(v.display_name, '')),
              to_tsvector('simple', concat_ws(' ', vc.display_number, vc.entity_number, vc.status, coalesce(v.display_name, ''))),
              lower(coalesce(vc.entity_number, vc.display_number)),
