    ...
    private static async Task SeedInvoiceDocumentsAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        await ExecuteCompanyProjectionStepAsync(
            ...
            select
              ...
              'transactions',
              i.invoice_number,
              concat_ws(' | ', i.status, coalesce(i.document_currency_code, ''), coalesce(c.display_name, '')),
              concat_ws(' ', i.invoice_number, i.entity_number, i.status, coalesce(c.display_name, '')),
              to_tsvector('simple', concat_ws(' ', i.invoice_number, i.entity_number, i.status, coalesce(c.display_name, ''))),
    ...
    private static async Task SeedCreditNoteDocumentsAsync(NpgsqlConnection connection, NpgsqlTransaction transaction, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        await ExecuteCompanyProjectionStepAsync(
            ...
            select
              ...
              concat_ws(' | ', note.status, coalesce(note.document_currency_code, ''), coalesce(c.display_name, '')),
              concat_ws(' ', note.credit_note_number, note.entity_number, note.status, coalesce(c.display_name, '')),
              to_tsvector('simple', concat_ws(' ', note.credit_note_number, note.entity_number, note.status, coalesce(c.display_name, ''))),
