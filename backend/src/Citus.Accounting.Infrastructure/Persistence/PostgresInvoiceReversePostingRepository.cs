...
/// <summary>
/// Postgres impl of <see cref="IInvoiceReversePostingRepository"/>. Reads
/// the original invoice JE (source_type='invoice', source_id=invoiceId)
/// plus its journal_entry_lines, then constructs an
/// <see cref="InvoiceReversePostingDocument"/> with each line's debit/
/// credit (and TxDebit/TxCredit) swapped. Reading the canonical JE row
/// gives the already-balanced amounts — including every per-rule sales-tax
/// leg — so the compensation reverses the whole posting exactly. Mirrors
/// <see cref="PostgresExpenseVoidPostingRepository"/>.
/// </summary>
public sealed class PostgresInvoiceReversePostingRepository : IInvoiceReversePostingRepository
{
    private const string ForwardSourceType = "invoice";
    private const string ReverseSourceType = "invoice_reversal";

    private readonly PostgresConnectionFactory _connections;
    private readonly PostgresExecutionContextAccessor _executionContextAccessor;

    public PostgresInvoiceReversePostingRepository(
    ...
    public async Task<InvoiceReversePostingPreparation> PreparePostingDocumentAsync(
        CompanyId companyId,
        UserId actorUserId,
        Guid invoiceId,
        CancellationToken cancellationToken)
    {
        ...
        // Idempotency: already-reversed invoice returns the prior reverse JE.
        var existing = await TryReadExistingReverseJournalAsync(scope, companyId, invoiceId, cancellationToken);
        if (existing is not null)
        {
            return new InvoiceReversePostingPreparation(
                Document: null,
                ExistingJournalEntryId: existing.Value.Id,
                ExistingJournalEntryDisplayNumber: existing.Value.DisplayNumber);
        }

        var header = await ReadForwardHeaderAsync(scope, companyId, invoiceId, cancellationToken);
        if (header is null)
        {
            throw new InvalidOperationException(
                $"Invoice {invoiceId:D} has no posted forward journal entry to reverse.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Forward journal entry {header.Value.JournalEntryId:D} for invoice {invoiceId:D} has no lines.");
        }
        ...
        var reverseLines = new List<InvoiceReversePostingDocumentLine>(lines.Count);
        var lineNumber = 1;
        foreach (var line in lines)
        {
            reverseLines.Add(new InvoiceReversePostingDocumentLine(
        ...
        var idShort = invoiceId.ToString("N")[..12].ToUpperInvariant();
        var ordinalSuffix = BitConverter.ToUInt32(invoiceId.ToByteArray(), 0) % (EntityNumber.MaxOrdinal + 1);
        var entityNumber = EntityNumber.Create(header.Value.PostingDate.Year, ordinalSuffix);
        var displayNumber = new DocumentNumber($"INVREV-{idShort}");

        var document = new InvoiceReversePostingDocument(
            id: invoiceId,
            companyId: companyId,
