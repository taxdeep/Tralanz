...
public sealed class PostgresArOpenItemRepository : IArOpenItemRepository
{
    ...
    public PostgresArOpenItemRepository(
    ...
    public async Task EnsureForInvoiceAsync(
    ...
    public async Task EnsureForCreditNoteAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (latestRequest is not null)
        {
            return new OpenItemAdjustmentRequestAttempt(
                ...
                $"A governed {latestRequest.AdjustmentType} request is already {latestRequest.RequestStatus} for this AR open item.",
                latestRequest);
        }
        ...
        return new OpenItemAdjustmentRequestAttempt(
            ...
            true,
            "request_recorded",
            "Governed AR open item adjustment request was recorded. Open-item truth and accounting entries remain unchanged until a Posting Engine-backed execution flow is implemented.",
            request);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.RequestStatus == "submitted")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "submit",
                "already_submitted",
                "The governed AR open item adjustment request is already submitted.");
        }
        ...
        if (request.RequestStatus == "cancelled")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "submit",
                "blocked_cancelled",
                "Cancelled AR open item adjustment requests cannot be submitted.");
        }
        ...
        return new OpenItemAdjustmentRequestTransitionResult(
            ...
            "submit",
            "submitted",
            "Governed AR open item adjustment request was submitted. Accounting execution is still blocked until a Posting Engine-backed flow exists.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (request.RequestStatus == "cancelled")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "cancel",
                "already_cancelled",
                "The governed AR open item adjustment request is already cancelled.");
        }
        ...
        if (request.ExecutionStatus is "execution_requested" or "journal_entry_posted")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "AR open item adjustment requests cannot be cancelled after execution has started.");
        }
        ...
        return new OpenItemAdjustmentRequestTransitionResult(
            ...
            "cancel",
            "cancelled",
            "Governed AR open item adjustment request was cancelled without changing open-item or accounting truth.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!request.RequiresApproval)
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "This AR open item adjustment request does not require approval.");
        }

        var authorityBlock = ValidateAdjustmentApprovalAuthority(request, actorId, "approve", "AR");
        ...
        if (request.RequestStatus != "submitted")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "Only submitted AR open item adjustment requests can be approved.");
        }
        ...
        if (request.ExecutionStatus is "execution_requested" or "journal_entry_posted")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "approve",
                "blocked_execution_started",
                "AR open item adjustment requests cannot be approved after execution has started.");
        }
        ...
        if (request.ApprovalStatus == "approved")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "approve",
                "already_approved",
                "The governed AR open item adjustment request is already approved.");
        }
        ...
        return new OpenItemAdjustmentRequestTransitionResult(
            ...
            "approve",
            "approved",
            "Governed AR open item adjustment request was approved. Posting execution may continue if current open-item truth still passes readiness.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var authorityBlock = ValidateAdjustmentApprovalAuthority(request, actorId, "reject", "AR");
        if (authorityBlock is not null)
        {
            return authorityBlock;
        }
        ...
        if (request.RequestStatus != "submitted")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "reject",
                "blocked_by_request_status",
                "Only submitted AR open item adjustment requests can be rejected.");
        }
        ...
        if (request.ExecutionStatus is "execution_requested" or "journal_entry_posted")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "reject",
                "blocked_execution_started",
                "AR open item adjustment requests cannot be rejected after execution has started.");
        }
        ...
        if (request.ApprovalStatus == "rejected")
        {
            return new OpenItemAdjustmentRequestTransitionResult(
                ...
                "reject",
                "already_rejected",
                "The governed AR open item adjustment request is already rejected.");
        }
        ...
        return new OpenItemAdjustmentRequestTransitionResult(
            ...
            "reject",
            "rejected",
            "Governed AR open item adjustment request was rejected. Open-item and accounting truth remain unchanged.");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("No active accounts receivable control account is configured for this AR adjustment.");
        }
        ...
        if (!readiness.IsAvailable || !readiness.PostingExecutionReady)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("AR open item adjustment amount no longer matches current open-item truth.");
        }
                         ...
                         """))
        {
            ...
            if (affectedRows != 1)
            {
                throw new InvalidOperationException("AR open item adjustment could not reduce the current open-item balance.");
            }
        }
        ...
        await AppendAdjustmentRequestTransitionAsync(
            ...
            new
            {
                ...
                CompensationSourceType = "ar_open_item_adjustment",
        ...
        return new OpenItemAdjustmentExecutionResult(
            ...
            nextStatus == "closed"
                ? "Governed AR open item adjustment posted through the Posting Engine and closed the open item."
                : "Governed AR open item adjustment posted through the Posting Engine and reduced the open item.",
            journalEntryId,
            journalEntryDisplayNumber,
            "ar_open_item_adjustment",
            adjustmentAmountTx,
        ...
        CancellationToken cancellationToken)
    {
        await using var command = scope.CreateCommand(
            ...
            select
              ...
              oi.source_id,
              coalesce(i.invoice_number, cn.credit_note_number, oi.source_id::text) as source_document_display_number,
              coalesce(i.status, cn.status, 'unknown') as source_document_status,
              coalesce(i.invoice_date, cn.credit_note_date, oi.due_date, current_date) as document_date,
              oi.due_date,
              oi.document_currency_code,
              oi.base_currency_code,
              oi.balance_side,
        ...
        decimal? adjustmentAmountTx)
    {
        ...
        if (adjustmentType is null)
        {
            return snapshot.ToPreview(
                ...
                false,
                "Only write_off and small_balance_adjustment are supported by the governed AR adjustment skeleton.");
        }
        ...
        if (snapshot.Status is not ("open" or "partially_applied"))
        {
            return snapshot.ToPreview(
                ...
                false,
                "Only open or partially applied AR open items can enter governed adjustment review.");
        }
        ...
        if (requestedAmounts.AmountTx > snapshot.OpenAmountTx)
        {
            return snapshot.ToPreview(
                ...
                "blocked_by_adjustment_amount",
                false,
                "Adjustment amount cannot exceed the current AR open-item balance.");
        }
        ...
        DateOnly asOfDate)
    {
        if (request.RequestStatus != "submitted")
        {
            return new OpenItemAdjustmentRequestReadiness(
                ...
                "blocked_by_request_status",
                false,
                "Governed AR open item adjustment execution requires a submitted request.");
        }
        ...
        if (request.RequiresApproval && request.ApprovalStatus != "approved")
        {
            var reason = request.ApprovalStatus == "rejected"
                ? "Governed AR adjustment was rejected. Cancel this request and create a new request if review should restart."
                : $"Governed AR adjustment requires approval before execution. Current approval status: {request.ApprovalStatus}.";
        ...
        return new OpenItemAdjustmentRequestReadiness(
            ...
            true,
            "Governed AR request and open-item truth are ready for Posting Engine-backed adjustment execution.");
    }
        ...
        return new OpenItemAdjustmentExecutionPlan(
            ...
            new[]
            {
                ...
                new OpenItemAdjustmentExecutionPlanStep(
                    ...
                    openItemReady
                        ? "The current AR open item is still eligible for adjustment review."
                        : readiness.Reason),
                new OpenItemAdjustmentExecutionPlanStep(
                    3,
                    "approval_gate",
                    ...
                    approvalReady
                        ? "Approval is satisfied for this AR adjustment request."
                        : readiness.Reason),
                new OpenItemAdjustmentExecutionPlanStep(
                    4,
                    ...
                    readiness.IsAvailable
                        ? "A governed AR open item adjustment source document can be built from the submitted request."
                        : readiness.Reason),
                new OpenItemAdjustmentExecutionPlanStep(
                    5,
                ...
                new OpenItemAdjustmentExecutionPlanStep(
                    ...
                    "Close or reduce AR open-item balance",
                    readiness.PostingExecutionReady ? "ready" : "blocked",
        ...
        CancellationToken cancellationToken)
    {
        if (!await IsStructurallyAllowedAdjustmentAccountAsync(scope, companyId, accountId, cancellationToken))
        {
            return new AdjustmentAccountPolicyResult(
                false,
                "The AR adjustment offset account is not an active governed adjustment account in the active company context.");
        }

        if (!await AdjustmentAccountMappingTableExistsAsync(scope, cancellationToken))
        {
            return new AdjustmentAccountPolicyResult(true, "No formal AR adjustment account mapping table exists yet; structural account guard was applied.");
        }

        var normalizedAdjustmentType = NormalizeAdjustmentType(adjustmentType);
        if (normalizedAdjustmentType is null)
        {
            return new AdjustmentAccountPolicyResult(false, "The AR adjustment type is not valid for account-mapping policy.");
        }
        ...
        if (!hasActiveMapping)
        {
            return new AdjustmentAccountPolicyResult(true, "No active AR adjustment account mapping exists; structural account guard was applied.");
        }
        ...
        return isMappedAccount
            ? new AdjustmentAccountPolicyResult(true, "The AR adjustment offset account matches active company/book adjustment account policy.")
            : new AdjustmentAccountPolicyResult(
                false,
                "The AR adjustment offset account is not mapped by active company/book adjustment account policy.");
    }
        ...
        return new OpenItemAdjustmentDocument(
            ...
            [
                new OpenItemAdjustmentDocumentLine(
                    ...
                    adjustmentAccountId,
                    snapshot.PartyId,
                    $"AR {request.AdjustmentType} for {snapshot.SourceDocumentDisplayNumber}",
                    request.RequestedAdjustmentAmountTx,
