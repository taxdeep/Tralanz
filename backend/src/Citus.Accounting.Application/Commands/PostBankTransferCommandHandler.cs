...
public sealed class PostBankTransferCommandHandler
{
    private readonly IBankTransferDocumentRepository _documents;
    private readonly IPostingEngine _postingEngine;
    private readonly IUnitOfWork _unitOfWork;

    public PostBankTransferCommandHandler(
        IBankTransferDocumentRepository documents,
    ...
    public Task<PostBankTransferCommandResult> HandleAsync(
        PostBankTransferCommand command,
        ...
        return _unitOfWork.ExecuteAsync(async ct =>
        {
            ...
            if (document is null)
            {
                throw new InvalidOperationException(
                    "Bank transfer document was not found in the active company context.");
            }
            ...
            var idempotencyKey = string.IsNullOrWhiteSpace(command.IdempotencyKey)
                ? $"bank_transfer:{command.CompanyId}:{command.DocumentId}"
            ...
            return PostBankTransferCommandResult.FromPostingResult(result);
        }, cancellationToken);
    }
}
